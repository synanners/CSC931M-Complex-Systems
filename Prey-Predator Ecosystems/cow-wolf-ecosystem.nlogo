...
to setup
  ...
  [
    ...
    set color black
    set size 2
    set energy random ( 2 * cow-energy-from-food)
  ...
  [
    ...
    set color red
    set size 1.4  ;
    set energy random (2 * wolf-energy-from-food)
    setxy random-xcor random-ycor
  ]
  reset-ticks
end
...
to go
  ...
  if not any? wolves and count cows > max-cows [ user-message "Too much cow level" stop ]
  ask cows [
    move
    set energy energy - 1
    eat-grass
...
to eat-grass
  if pcolor = green [
    set pcolor white
    set energy energy + cow-energy-from-food
  ]
end
...
"grass / 4" 1.0 0 -12087248 true "" "plot count grass / 4"
